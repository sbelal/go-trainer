/**
 * Go-Trainer Verification Engine
 *
 ...
 * @returns {{ phasePath: string, phaseName: string, folderName: string }}
 */
function resolvePhase(phaseNumber) {
  const repoDir = path.resolve(__dirname, '..');
  const phasesDir = path.join(repoDir, 'phases');
  ...
  // e.g. "01-basics-and-syntax" → "Basics And Syntax"
 ...
 * @param {string} phasePath - Absolute path to the phase folder
 * @returns {{ passed: boolean, output: string, error: string }}
 */
function runCheck(check, phasePath) {
  if (check.type === 'command' || check.type === 'script') {
    return runCommandCheck(check, phasePath);
  }
...
function printResults(phaseNumber, phaseName, folderName, results) {
  ...
  for (const { check, result } of results) {
    if (result.passed) {
      ...
      if (result.error) {
        console.log(`     ${color.red('Error:')} ${result.error.trim().split('\n').join('\n            ')}`);
      }
      if (check.fix) {
...
function verifyPhase(phaseNumber) {
  ...
  for (const check of checks) {
    const result = runCheck(check, phasePath);
